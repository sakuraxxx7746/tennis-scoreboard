package com.example.tennis_scorebord
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // --- 初期化ボタン ---
        val resetButton = createBorderedButton("初期化") {
            // カスタムビュー（メッセージ + チェックボックス）を作る
            ...
            AlertDialog.Builder(this@MainActivity)
                ...
                .setPositiveButton("OK") { _, _ ->
                    resetAll(clearSignatures = check.isChecked)
    ...
    private fun createBorderDrawable(): android.graphics.drawable.Drawable {
        // 透明な背景（枠線を表示しない）を返す
        return android.graphics.drawable.ColorDrawable(Color.TRANSPARENT)
    }
    ...
    private fun createScoreTextView(initial: String = "0"): TextView {
        return object : TextView(this) {
            ...
            override fun onDraw(canvas: Canvas) {
                ...
                val offset = height * 0.13f  // digital-7フォントを視覚的に中央に配置するため調整
    ...
    private fun resetAll() {
        resetAll(clearSignatures = true)
    }

    // clearSignatures: true のときサインもデフォルト状態に戻す
    private fun resetAll(clearSignatures: Boolean = false) {
        for (row in 0..1) {
            for (col in 0..2) {
